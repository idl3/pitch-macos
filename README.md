...
- macOS 14.2+
- Swift 5.9+ / Xcode 15+
- First launch will ask for **System Audio Recording** TCC permission.
- For distribution, an **Apple Developer ID** code-signing certificate is strongly recommended (see below).
...
- A pitch-shift end-to-end test that feeds a 440 Hz tone through `AVAudioUnitTimePitch` and verifies the output is ~880 Hz.
- Vocal-removal unit tests for passthrough, mono cut, karaoke, dual mono, and blend scaling.
...
For a signed build (preferred for Gatekeeper and TCC stability):

```bash
cd KBear
CODESIGN_ID="Developer ID Application: Your Name (Team ID)" ./scripts/create_dmg.sh
```

You can also notarize the DMG afterward with `xcrun notarytool`.
...
3. **Permissions**: verify the first-run TCC prompt for System Audio Recording; graceful handling when denied.
...
- On macOS Sequoia and later, `CGPreflightScreenCaptureAccess()` only returns `true` for binaries signed with a valid Apple Developer ID certificate. KBear uses the `kTCCServiceAudioCapture` TCC SPI instead, which is the correct service for CoreAudio process taps, but a stable signing identity is still required for the TCC grant to persist across rebuilds.
