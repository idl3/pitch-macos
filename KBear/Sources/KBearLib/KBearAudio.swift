...
final class KBearAudio: ObservableObject {
    ...
    private let permission = KBearAudioPermission.shared
    ...
    private func updateEngine() {
        ...
        if shouldRun {
            ...
            Task { @MainActor [weak self] in
                ...
                let granted = await self.ensureAudioCapturePermission()
    ...
    private func startEngine() {
        guard pitchEnabled || removeVocalsEnabled else { return }
    ...
    private func ensureAudioCapturePermission() async -> Bool {
        if permission.isAudioCaptureAuthorized {
            return true
        }
        statusText = "Allow System Audio Recording to use KBear"
        permissionPromptHandler?()
        let granted = await permission.requestAudioCapturePermission()
        if granted {
            statusText = ""
            permissionGrantedHandler?()
            return true
        } else {
            statusText = "Permission denied. Enable System Audio Recording for KBear in System Settings."
            needsRestart = true
            return false
        }
    }
    ...
    private func setup() throws {
        ...
        sourceNode = AVAudioSourceNode(format: format) { [ring, audioParams] silence, time, frameCount, outputData -> OSStatus in
            ...
            let vocalParams = VocalBlendParameters(
                mono: audioParams.vocalMono,
                karaoke: audioParams.vocalKaraoke,
                dualMono: audioParams.dualMonoOutput,
                volume: audioParams.volume
            )
            for i in 0..<Int(framesToCopy) {
                let l = src[i * 2]
                let r = src[i * 2 + 1]
                let out = processStereoSample(l: l, r: r, params: vocalParams)
                left[i] = out.left
                right[i] = out.right
            }
